...
from error import export_error_cases
...
def main():
    ...
    parser.add_argument('--emb_size', default=768, type=int, help='embedding dimensions')
    ...
    parser.add_argument('--pos_weight', default=15.0, type=float, help='the weight of bce loss')
...
def test_epoch(data_loader, model, args):
    ...
    for batch_idx, (query_imgs, rs_imgs, mat_clickxy, ori_gt_bbox, _) in enumerate(data_loader):
        ...
        accu_list, accu_center, iou, each_acc_list, pred_bboxes, iou_list = eval_iou_acc(pred_anchor, ori_gt_bbox, anchors_full, best_anchor_gi_gj[:, 1], best_anchor_gi_gj[:, 2], args.img_size, iou_threshold_list=[0.5, 0.25])
        ...
        # --- GỌI HÀM LƯU ẢNH LỖI TỪ FILE NGOÀI ---
        error_count = export_error_cases(
            query_imgs=query_imgs,
            rs_imgs=rs_imgs,
            ori_gt_bbox=ori_gt_bbox,
            pred_bboxes=pred_bboxes,
            pred_coords=pred_coords,
            args=args,
            error_count=error_count,
            max_errors=100,   # Tối đa lưu 100 ảnh lỗi
            iou_thresh=0.5   # Ngưỡng IoU < 0.5 coi như đoán sai
        )
        # measure elapsed time
        batch_time.update(time.time() - end)
        end = time.time()
