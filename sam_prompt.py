...
for split_name in ['val', 'test']:
    ...
    for data in tqdm(data_list):
        ...
        b, c, h, w = rsimg.shape
        prompt_points = torch.cat([(pred_bbox[:, 0] + pred_bbox[:, 2])/2., (pred_bbox[:, 1] + pred_bbox[:, 3])/2.], dim=-1).unsqueeze(0).unsqueeze(0)
        ...
        masks = F.interpolate(low_res_masks, scale_factor=4, mode='bilinear', align_corners=False)
        ...
        vis_rsimg_name = "{}--{}--bbox({},{},{},{}).jpg".format(queryimg_name[:-4], rsimg_name[:-4], int(bbox[0]), int(bbox[1]),
